...
class IDetect_att2(nn.Module):
    ...
    def __init__(self, n_classes=5, anchors=(), ch=()):  # detection layer
        super(IDetect_att2, self).__init__()
        self.n_classes = n_classes  # number of classes
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                print("\ninference with sigmoid/softmax")
                #y = x[i].softmax(-1)#
                y = x[i].sigmoid()
                #y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]  # xy
                #y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
    ...
    def fuseforward(self, x):
        print("IDetect_att2.fuseforward")
        # x = x.copy()  # for profiling
        z = []  # inference output
        self.training |= self.export
        for i in range(self.nl):
        ...
        if self.training:
            ...
            print("Error,IDetect_att2.convert")
            z = self.convert(z)
            out = (z, )
        elif self.concat:
            out = torch.cat(z, 1)            
...
class IDetect_att3(nn.Module):
    ...
    def __init__(self, n_classes=2, anchors=(), ch=()):  # detection layer
        super(IDetect_att3, self).__init__()
        self.n_classes = n_classes  # number of classes
        self.no = n_classes  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
    ...
    def fuseforward(self, x):
        print("IDetect_att3.fuseforward")
        # x = x.copy()  # for profiling
        z = []  # inference output
        self.training |= self.export
        for i in range(self.nl):
        ...
        if self.training:
            ...
            print("Error,IDetect_att3.convert")
            z = self.convert(z)
            out = (z, )
        elif self.concat:
            out = torch.cat(z, 1)            
...
class IDetect_att4(nn.Module):
    ...
    def __init__(self, n_classes=4, anchors=(), ch=()):  # detection layer
        super(IDetect_att4, self).__init__()
        self.n_classes = n_classes  # number of classes
        self.no = n_classes  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
    ...
    def fuseforward(self, x):
        print("IDetect_att4.fuseforward")
        # x = x.copy()  # for profiling
        z = []  # inference output
        self.training |= self.export
        for i in range(self.nl):
        ...
        if self.training:
            ...
            print("Error,IDetect_att4.convert")
            z = self.convert(z)
            out = (z, )
        elif self.concat:
            out = torch.cat(z, 1)            
...
class IKeypoint(nn.Module):
    ...
    def __init__(self, n_classes=80, anchors=(), nkpt=17, ch=(), inplace=True, dw_conv_kpt=False):  # detection layer
...
class IBin(nn.Module):
    ...
    def __init__(self, n_classes=80, anchors=(), ch=(), bin_count=21):  # detection layer
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, anchors=None):  # model, input channels, number of classes
        ...
        #self.classes = [str(i) for i in range(self.yaml['n_classes_lis'][0])]  # default names
    ...
    def _initialize_aux_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, mi2, s in zip(m.m, m.m2, m.stride):  # from
            b = mi.bias.view(m.na, -1)  # conv.bias(255) to (3,85)
            b.data[:, 4] += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
            b.data[:, 5:] += math.log(0.6 / (m.n_classes_lis[0] - 0.99)) if cf is None else torch.log(cf / cf.sum())  # cls
            mi.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
            b2 = mi2.bias.view(m.na, -1)  # conv.bias(255) to (3,85)
            b2.data[:, 4] += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
            b2.data[:, 5:] += math.log(0.6 / (m.n_classes_lis[0] - 0.99)) if cf is None else torch.log(cf / cf.sum())  # cls
    ...
    def _initialize_biases_bin(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b[:, (obj_idx+1):].data += math.log(0.6 / (m.n_classes_lis[0] - 0.99)) if cf is None else torch.log(cf / cf.sum())  # cls
    ...
    def autoshape(self):  # add autoShape module
        ...
        copy_attr(m, self, include=('yaml', 'n_classes_lis', 'hyp', 'names', 'stride'), exclude=())  # copy attributes
        return m
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    anchors, n_classes,n_classes_lis,gd, gw = d['anchors'], d['n_classes_lis'][0], d['n_classes_lis'], d['depth_multiple'], d['width_multiple']
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors
    no = na * (n_classes + 5)  # number of outputs = anchors * (classes + 5)
